...
class OverviewContainer extends Component {
	...
	getByDay = () => {
		...
		fetch(`http://api.bcounter.club/api/budgetEntry/get/day/${month}/${day}/${year}`, {
			headers: {
	...
	getByWeek = () => {
		...
		fetch(`http://api.bcounter.club/api/budgetEntry/get/week/${week}/${year}`, {
			headers: {
				"authorization": `Bearer ${localStorage.uid}`
			}
		})
	...
	getByMonth = () => {
		let month = moment().format('M')
		let year = moment().format('Y')
		fetch(`http://api.bcounter.club/api/budgetEntry/get/month/${month}/${year}`, {
			headers: {
				"authorization": `Bearer ${localStorage.uid}`
			}
		})
